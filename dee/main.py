...
class LoanManager:
    def __init__(self, root):
        ...
        # Create a label and entry field for the loan type
        lbl_ltype = Label(Detail_F, text="Loan Type",
                         font=("Comic Sans MS", 12,))
        lbl_ltype.grid(row=9, column=0, pady=10, padx=20, sticky="w")
        txt_ltype = Entry(Detail_F, font=("Comic Sans MS", 10, ), bd=3, textvariable=self.ltype)
        txt_ltype.grid(row=9, column=1, pady=10, sticky="w")

        # Create a label and entry field for the monthly payment
        lbl_Mp = Label(Detail_F, text="Monthly Payment",
                       font=("Comic Sans MS", 12,))
        lbl_Mp.grid(row=10, column=0, pady=10, padx=20, sticky="w")
        txt_Mp = Label(Detail_F, font=("Comic Sans MS", 10,),
                       bd=3, state=DISABLED, textvariable=self.mpay)
        txt_Mp.grid(row=10, column=1, pady=10, sticky="w")
        # Create a label and entry field for the total payment
        lbl_tp = Label(Detail_F, text="Total Payment",
                       font=("Comic Sans MS", 12,))
        lbl_tp.grid(row=11, column=0, pady=10, padx=20, sticky="w")
        txt_tp = Label(Detail_F, font=("Comic Sans MS", 10,),
                       bd=3, state=DISABLED, textvariable=self.tpay)
        txt_tp.grid(row=11, column=1, pady=10, sticky="w")
        recordFrame = Frame(self.root, bd=5, relief=RIDGE)
        recordFrame.place(x=535, y=100, width=720, height=530)
        ...
        self.employee_table = ttk.Treeview(recordFrame, columns=(
            "empId", "name", "years", "rate", "ltype", "Mpayment", "Tpayment", "mobile"), yscrollcommand=yscroll.set)
        ...
        self.employee_table.heading("ltype", text="Loan Type")
        ...
        self.employee_table.column("ltype", width=100)
        ...
        btnFrame.place(x=700, y=530, width=480, height=60)
    ...
    def validate_input(self):
        """Validates name, mobile number, and pin code fields."""
        ...
        if not (pin_code.isdigit() and len(pin_code) == 6):
            messagebox.showerror("Error", "Pin code should be 6 digits")
            return False
    ...
    def addrecord(self):
        if not self.validate_input():
            return 
        # Access loan_type as self.loan_type.get()
        print(self.ltype.get())  # or add this value to the database
    ...
    def fetch_data(self):
        ...
        cur = con.cursor()
        cur.execute(
            "select Loan_Id , Name , Year , Rate , Loan_Type , Monthly_Payment , Total_Payment , MobileNumber , AadharNumber , Address , Pincode , Amount from customer")
        rows = cur.fetchall()
        if len(rows) != 0:
    ...
    def update(self):
        if not self.validate_input():
            ...
            cur = con.cursor()
            cur.execute(
                "update customer set Name = ?, MobileNumber = ?, AadharNumber = ?, Address = ?, Pincode = ?, Amount = ?, Year = ?, Rate = ?, Loan_type = ?, Total_Payment = ?, Monthly_Payment = ? where Loan_Id = ?",
                (
                    self.name.get(),
                    self.mob.get(),
                    self.aadhar.get(),
    ...
    def get_cursor(self, ev):
        ...
        self.year.set(row[2])
        self.rate.set(row[3])
        self.ltype.set(row[4])
        self.mpay.set(row[5])
        self.tpay.set(row[6])
        self.mob.set(row[7])
        self.aadhar.set(row[8])
        self.add.set(row[9])
        self.pin.set(row[10])
        self.amount.set(row[11])
...
# The table has 10 columns: Loan_Id, Name, MobileNumber, AadharNumber, Address, Pincode, Amount, Year, Rate, Loan_type and Monthly_Payment, Total_Payment
# The Loan_Id column is set as the primary key
# Drop the existing table (if it exists)
cur.execute("DROP TABLE IF EXISTS customer")

# Recreate the table with the new schema including Loan_type
cur.execute('''
